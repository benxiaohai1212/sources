...
1. 使用mysqladmin flush-hosts命令清除缓存；终端输入`mysqladmin  -u  root  -p  flush-hosts`  
2. 修改最大链接数  
  i. 查看最大连接数`show variables like '%max_connections%'`;  
  ii. 修改方法：  
    > 打开MySQL配置文件`my.ini`或`my.cnf`查找`max_connections=100`修改为`max_connections=1000`服务里重起MySQL  
    > 命令行登录MySQL执行`MySQL> set global max_connections=200`，此方法重启MYSQL后失效  
3. 为TABLE增加列：  
 `ALTER TABLE company ADD COLUMN zip_code VARCHAR(6) NULL COMMENT '邮政编码' AFTER distinction;`  
4. 查itemfile表中itemdesc、re_level列的信息，要求re_level为NULL时显示为0，写出相应语句：  
  `select itemdesc,ifnull(re_level,’0’) from itemfile;`

5、[Err] 1055 - Expression #1 of ORDER BY clause is not in GROUP BY clause and contains nonaggregated column 'information_schema.PROFILING.SEQ' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
```
